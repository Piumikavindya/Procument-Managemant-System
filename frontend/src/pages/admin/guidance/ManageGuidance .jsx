...
const ManageGuidance = () => {
  ...
  const currentItems = filteredGuidances.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  const handleViewPdf = async (guidanceId) => {
    try {
      const response = await axios.get(
        `http://localhost:8000/viewPdf/${guidanceId}`,
        {
    ...
    <div className="p-4">
      ...
      <Breadcrumb
        crumbs={[
          ...
          { label: "Manage Guidance", link: `/ManageGuidance/${id}` },
        ]}
